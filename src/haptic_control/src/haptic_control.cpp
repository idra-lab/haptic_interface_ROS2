...
void HapticControl::update_current_ee_pos() {
  try {
    auto trans = tf_buffer_->lookupTransform(tool_link_name_, base_link_name_,
                                             tf2::TimePointZero);
...
void HapticControl::ImpedanceThread() {
  ...
  // Scale the rotation
  Eigen::Quaterniond qScaledDiff =
      Eigen::Quaterniond::Identity().slerp(scaling_factor, qDiff);
  ...
  // Eigen::Quaterniond qTarget = qScaledDiff.conjugate() * qEEStart; // EEStart
  // - diff = EEStart + (cur - start)
  qTarget.normalize();
  ...
  if (this->bunding_box) {
    target_pose.pose.position.x =
        std::clamp(target_pose.pose.position.x, bounding_box_centre[0] - 0.015,
                   bounding_box_centre[0] + 0.015);
    target_pose.pose.position.y =
        std::clamp(target_pose.pose.position.y, bounding_box_centre[1] - 0.015,
                   bounding_box_centre[1] + 0.015);
    target_pose.pose.position.z =
        std::clamp(target_pose.pose.position.z, bounding_box_centre[2] - 0.015,
                   bounding_box_centre[2] + 0.015);
  }

  // publish
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"),
              "PUBLISHING TARGET POSITION: %f %f %f",
  ...
  if (ctr % 1000 == 0) {
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Current position: %f %f %f",
                target_pose.pose.position.x, target_pose.pose.position.y,
                target_pose.pose.position.z);
  }
}
