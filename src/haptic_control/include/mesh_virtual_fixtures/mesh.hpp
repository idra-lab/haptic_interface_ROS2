...
#pragma omp parallel for
  ...
  void extrude_radially(const Eigen::Vector3d &origin, const double scale) {
    ...
    for (size_t i = 0; (int)i < num_faces; ++i) {
      // const Eigen::Vector3d face_center = (vertices[faces[i][0]] +
      // vertices[faces[i][1]] + vertices[faces[i][2]]) / 3; Eigen::Vector3d
      // direction = (origin - face_center).normalized();
  ...
  Eigen::Matrix4d compute_triangle_xfm(int idx) {
    ...
    const Eigen::Vector3d zaxis =
        ((v3 - v1).normalized()).cross(yaxis).normalized();
...
#pragma omp critical
    ...
    save_adjacency_dict(
        "adjacency_dict" + std::to_string(faces.size()) + ".txt",
        adjacency_dict);
