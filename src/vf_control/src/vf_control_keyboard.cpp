...
void VFControl::Initialize() {
  ...
  if (mesh_type_ == "file") {
    for (size_t i = 0; i < o3d_mesh->triangles_.size(); i++) {
      ...
      auto triangle_center =
          (o3d_mesh->vertices_[v0] + o3d_mesh->vertices_[v1] +
           o3d_mesh->vertices_[v2]) /
          3;
  ...
  Eigen::Vector3d delta_x =
      enforce_virtual_fixture(*mesh_, x_new_, vf_pose_, radius_,
                              constraint_planes_, lookup_area_, *visualizer_);
